 ...
 *
 * 동작 흐름: Istio Gateway (Envoy) → AuthorizationPolicy(CUSTOM) → ext_authz filter → 이
 * 서버:9001 (gRPC) → X-Demo-User 헤더 검증 → 허용: OkHttpResponse 에 X-Authz-User / X-Authz-Result
 * 헤더를 담아 반환 (Envoy 가 upstream 요청에 주입) → 차단: DeniedHttpResponse(403) 반환 (Envoy 가 upstream
 * 호출 없이 즉시 403 응답)
 *
