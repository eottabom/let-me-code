...
class RedisDnsCacheApplicationTests {
	...
	void inspectUnknownHostDoesNotThrow() {
		...
		assertThatCode(() -> inspector.inspect("redis-host.invalid")).doesNotThrowAnyException();
	}

	@Test
	void mainWithoutArgumentsDoesNotThrow() {
		assertThatCode(() -> RedisDnsCacheApplication.main(new String[0])).doesNotThrowAnyException();
	}
	...
	void connectWithNettyResolverDoesNotThrow() {
		...
		assertThatCode(() -> probe.connect("redis-host.invalid", 6379)).doesNotThrowAnyException();
	}
	...
	void connectWithJvmResolverDoesNotThrow() {
		...
		assertThatCode(() -> probe.connectWithJvmResolver("redis-host.invalid", 6379)).doesNotThrowAnyException();
	}
