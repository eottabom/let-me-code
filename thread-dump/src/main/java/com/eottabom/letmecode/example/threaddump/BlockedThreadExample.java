 ...
 *
 * Thread-1 holds the monitor on SharedResource and sleeps for a long time.
 * Thread-2 through Thread-5 try to enter the same synchronized block
 * and become BLOCKED waiting for the monitor.
 *
 * Run this class, then capture a thread dump with:
 *   jstack <pid>
 * You will see threads in state BLOCKED (on object monitor).
 */
